...
class CredentialService {
  ...
  /**
   * Fetch a membership witness from Certs
   * TODO: This is currently blocked by authentication
   * @param param0 
   */
  async fetchMembershipWitness({ credential }) {
    const accumulatorId = this.getAccumulatorId({ credential });
    const revocationId = credential.credentialStatus.revocationId;
    
    let certsApiUrl;

    if (credential.id && credential.id.indexOf('.dock.io')) {
      const origin = /^(https?:\/\/[^\/]+)/.exec(credential.id);
      if (origin && origin[0]) {
        certsApiUrl = origin[0].replace('creds-', 'api-');
      }
    }

    if (!certsApiUrl) {
      throw new Error('Unable to determine the API URL for the credential');
    }

    const endpoint = `/revocationStatus/${accumulatorId}/${revocationId}/witness`;

    const {data} = await axios.get(`${certsApiUrl}${endpoint}`, {
      headers: {
      }
    });
  ...
  async deriveVCFromBBSPresentation(params) {
    ...
    for (const {attributesToReveal} of credentials) {
      ...
      if (accumulatorId) {
        ...
        const witness = params.membershipWitness || await this.fetchMembershipWitness({ credential: credentials[0].credential });
        if (witness) {
