...
import {
  ...
  getBbsPlusPresentation,
  setBbsPlusPresentation,
} from './service';
...
describe('DID Service', () => {
  beforeAll(async () => {
    await getTestWallet();
    const props = Object.getOwnPropertyNames(BbsPlusPresentation.prototype);
    const mockBbsPlusPresentation = {};
    for (const prop of props) {
      mockBbsPlusPresentation[prop] = jest.fn();
    }
    setBbsPlusPresentation({
      ...mockBbsPlusPresentation,
      addCredentialsToPresent: jest.fn(() => 0),
    });
  });
  it('ServiceRpc', () => {
  ...
  it('expect to create bbs+ presentation', async () => {
    ...
    const credential = {
      ...
      issuer: {id: 'did:dock:5GyqRumR91iqwwRdLPBMqCtfo228htoB7isUS9ydYY3poGo6'},
      proof: {
        type: 'Bls12381BBS+SignatureDock2022',
        proofValue:
          'KcRsKfZAHfN9JgTv75huDUhrPggTKXvXqiPgD4TV4r6XN5mcDoqqFxmiquky4G3B2QvEdtU5f24XEG3v17y2fAy8ddguwbm3kygrChLTHvdVBYmt72j6USfuvEVHkvUhEAtaju8uKyVZc6Eait4ekffh1',
      },
    };
    ...
    const bbsPresentation = getBbsPlusPresentation();
    expect(bbsPresentation.addCredentialsToPresent).toBeCalledWith(credential);
