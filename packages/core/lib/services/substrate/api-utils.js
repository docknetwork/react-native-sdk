...
export const mapEventToErrorMessage = ({ event }) => {
  ...
  if (error.isModule) {
    ...
    const decoded = dockService.dock.api.registry.findMetaError(
      error.asModule,
    );
...
export const getExtrinsicErrors = ({ status, events }) => events
  .filter(extrisicErrorsFilter)
  .map(mapEventToErrorMessage)
