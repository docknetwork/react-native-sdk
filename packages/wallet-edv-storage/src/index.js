...
import { EDVHTTPStorageInterface, EDVWallet } from '@docknetwork/universal-wallet';
...
export async function deriveKeyAgreementKey(masterKeypair) {
  const keyDoc = await masterKeypair.export({ publicKey: true, privateKey: true });
  const verificationKey2020 = await Ed25519VerificationKey2020.fromEd25519VerificationKey2018({ keyPair: keyDoc });
...
export async function createOrGetEDVWallet(edvKeyBuffer, edvUrl, edvAuthKey) {
    ...
    const masterKeypair = await Ed25519VerificationKey2018.generate({ seed: edvKeyBuffer });
    ...
    const keys = {
      ...
      let walletId;
      const existingConfigLegacy = await storageInterface.findConfigFor(controller, 'primary');
      if (!existingConfigLegacy) {
        const referenceId = 'tcrinternal';
        const existingConfig = await storageInterface.findConfigFor(controller, referenceId);
        if (!existingConfig) {
